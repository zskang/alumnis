...
public class EasemobChatGroups {
	 ...
	 * @return
	 */
	public static ObjectNode changeGroupOwner(String chatgroupid,ObjectNode dataObjectNode) {
		ObjectNode objectNode = factory.objectNode();
		// check appKey format
		if (!HTTPClientUtils.match("^(?!-)[0-9a-zA-Z\\-]+#[0-9a-zA-Z]+", APPKEY)) {
			LOGGER.error("Bad format of Appkey: " + APPKEY);
		...
		if (StringUtils.isBlank(chatgroupid.trim())) {
			LOGGER.error("Property that named chatgroupid must be provided .");
			objectNode.put("message", "Property that named chatgroupid must be provided .");
			return objectNode;
		}
		if (!dataObjectNode.has("newowner")) {
			LOGGER.error("Property that named newowner must be provided .");
			objectNode.put("message", "Property that named newowner must be provided .");
			return objectNode;
		}
		try {
			URL allMemberssByGroupIdUrl = HTTPClientUtils.getURL(Constants.APPKEY.replace("#", "/") + "/chatgroups/"
	 ...
	 * @return
     */
	public static ObjectNode getGroupsByUsername(String username) {
		ObjectNode objectNode = factory.objectNode();
		// check appKey format
		if (!HTTPClientUtils.match("^(?!-)[0-9a-zA-Z\\-]+#[0-9a-zA-Z]+", APPKEY)) {
			LOGGER.error("Bad format of Appkey: " + APPKEY);
		...
		try {
			URL allMemberssByGroupIdUrl = HTTPClientUtils.getURL(Constants.APPKEY.replace("#", "/") + "/users/"
