...
/**
 * 记录日志到数据库中 数据库中type字段值对应 1:info 2:debug 3:error
 * 
...
public class LogService extends BaseService<Log> {
	...
	@Async
	@Transactional
	public void info(String mname, String msg, String username, String ip) {
	...
	@Async
	@Transactional
	public void debug(String mname, String msg, String username, String ip) {
		Throwable ex = new Throwable();
		StackTraceElement[] stackElements = ex.getStackTrace();
		int index = 0;
		for (int i = 0; i < stackElements.length; i++) {
	...
	@Async
	@Transactional
	public void error(String mname, String msg, String username, String ip) {
		Throwable ex = new Throwable();
		StackTraceElement[] stackElements = ex.getStackTrace();
		int index = 0;
		for (int i = 0; i < stackElements.length; i++) {
	...
	/***
	 * 调用当前栈的属性获取上一级调用日志方法的类名。 获取的字符串是org.xx.xx.xx
	 * 
	 * @param canme
	 *            类名
	 * */
	 ...
	 * 分页获取日志对象
	 * */
	public Map<String, Object> getLogList(PageBean pageBean, String name, String value) {
	...
	public JSONObject executeDelete(Integer[] ids, User user) {
		...
		logger.info("删除日志。操作人:" + user.getUserName() + "。操作时间:" + sdf.format(new Date()) + "");
