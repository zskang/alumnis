...
import com.nethsoft.core.util.ReflectUtil;
...
public class BaseHibernateDaoImpl extends BaseHibernateDao{
	 ...
	 * @param mList
	 */
	public <M> void merge(Collection<M> mList) {
		Session session = getSessionFactory().openSession();
		session.beginTransaction();
		for (M m : mList) {
			session.merge(m);
	...
	/**
	 * 根据ID获取对象 先查询缓存，没有的话，在查询数据库，没有数据则返回null
	 * 
	 * @param <M>
	 * @param clazz
	 * @param id
	...
	/**
	 * 根据ID获取对象 先查询一级缓存，不存在的话再查询二级缓存，不存在的话查询数据库，数据库中没有数据则抛异常
	 * 
	 * @param <M>
	 * @param clazz
	 * @param id
	...
	public <M> List<M> list(Class<M> clazz, PageBean pageBean) {
		...
		if (pageBean.getCriterions().size() > 0) {
			Map<String, Criteria> asMap = new HashMap<String, Criteria>();
			for (Criterion criterion : pageBean.getCriterions()) {
				_resloveCriterion(criteria, criterion, asMap, true);
			}
		}
		return criteria.list();
	}
	...
	/**
	 * 获取集合 分页 仅获取对象中的某些字段 其余字段为null值
	 * 
	 * @param clazz
	 * @param 需要查询字段
	 * @return
	...
	/**
	 * 根据SQL语句获取集合 返回数据结构为每行一个MAP集合，map集合的键为数据库字段名，值为字段值
	 * 
	 * @param sql
	 * @return
	 * @see queryForList
	...
	public List<Map<String, Object>> listByNativeSql(String sql, PageBean pageBean) {
		...
		if (pageBean.getPage() <= 1)
			...
			query.setFirstResult((pageBean.getPage() - 1) * pageBean.getRows());
		setParameters(query, pageBean.getParams());
		return query.list();
	}
	...
	public Object queryForObject(String sql) {
		return getByNativeSqlForObject(sql);
	}
	 ...
	 * @param criterion
	 */
	private void _resloveCriterion(Criteria criteria, Criterion criterion, Map<String, Criteria> asMap, boolean append) {
		if (criterion instanceof SimpleExpression ||
			...
			String propName = ReflectUtil.getFieldValue(criterion, "propertyName") + "";
			_resloveCriterionAlias(criteria, asMap, propName);
		}else if(criterion instanceof PropertyExpression){
			PropertyExpression exp = (PropertyExpression) criterion;
			String propName = ReflectUtil.getFieldValue(exp, "propertyName") + "";
			String otherPropName = ReflectUtil.getFieldValue(exp, "otherPropertyName") + "";
			
			_resloveCriterionAlias(criteria, asMap, propName);
			_resloveCriterionAlias(criteria, asMap, otherPropName);
		}else if(criterion instanceof LogicalExpression){
			LogicalExpression exp = (LogicalExpression)criterion;
			
			Criterion lhs = (Criterion) ReflectUtil.getFieldValue(exp, "lhs");
			Criterion rhs = (Criterion) ReflectUtil.getFieldValue(exp, "rhs");
			
			_resloveCriterion(criteria, lhs, asMap, false);
			_resloveCriterion(criteria, rhs, asMap, false);
		}else if(criterion instanceof Disjunction){ // or
			Disjunction exp = (Disjunction) criterion;
			
			Iterator<Criterion> iter = exp.conditions().iterator();
			while(iter.hasNext()){
				_resloveCriterion(criteria, iter.next(), asMap, false);
			}
			...
			_resloveCriterion(criteria, (Criterion)ReflectUtil.getFieldValue(exp, "criterion"), asMap, false);
		}
		if(append)
			criteria.add(criterion);
	}
	 ...
	 * @param propName
	 */
	private void _resloveCriterionAlias(Criteria criteria, Map<String, Criteria> asMap, String propName) {
